...
def wait(ref_pos=(1, 94), ref_val=(195, 36, 50), delay=0.3, timeout=60, skip_pcq=False):
...
def main():
    current_id = '020-74008'
    folder = 'C:/Users/tp-laizy/Downloads/saq_box2'
    filepath = os.path.join(folder, current_id + '.txt')
    pages = get_data(filepath)

    if pages[0][1] != current_id:
        print('Calling ID:', current_id)
        print('Data ID:', pages[0][1])
        print('\n*** Possible ID mismatch found. Please check! ***\n')
        breakpoint()
