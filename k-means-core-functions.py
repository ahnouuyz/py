...
def k_means(raw_data_2D, k=2, max_iter=10):
    initial_coords = random.sample(set(np.ndindex(raw_data_2D.shape)), k=k)
    centroids = np.array(initial_coords)
    cluster_dct_frames = []
    scores_frames = []
    
    for iteration in range(max_iter):
        cluster_dct = assign_points_to_clusters(raw_data_2D, centroids)
...
def animate_k_means(cluster_dct_frames):
    ...
    sns.heatmap(raw_data_2D, cmap="BuGn", square=True, ax=ax, alpha=1, cbar=False)
